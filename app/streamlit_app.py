...
st.markdown('<div class="hero"><h1>📈 Trading Assistant</h1><p>Multi-market research · fundamentals · analyst consensus · technicals · multi-factor scoring · risk scenarios</p></div>', unsafe_allow_html=True)
...
def get_balance_sheet(symbol: str) -> pd.DataFrame:
    """Cache the slower fundamental statement request."""
    try:
...
if not analysis:
    ...
    st.markdown("Selectezi o companie și primești într-un singur loc prețul, indicatorii tehnici, evaluarea companiei, date de bilanț, consensul analiștilor, scorul multi-factor și simulatorul de risc.")
    st.stop()
...
m2.metric("Research score", f"{ai['score']:.0f}/100", f"{ai['label']} · {ai['confidence']}")
...
for col, label, key in zip(score_cols, ["Technical", "Fundamental", "Analyst", "Valuation", "Risk"], ["technical", "fundamental", "analyst", "valuation", "risk"]):
    col.metric(label, f"{ai[key]:.0f}/100")
...
with chart_tab:
    ...
    for enabled, col, name in [(show_sma20, "SMA_20", "SMA 20"), (show_sma50, "SMA_50", "SMA 50"), (show_sma200, "SMA_200", "SMA 200")]:
        if enabled:
            fig.add_trace(go.Scatter(x=analyzed["Date"], y=analyzed[col], name=name, mode="lines"))
    ...
    c.metric("ATR %", f"{indicators['atr_percent']:.2f}%" if indicators.get("atr_percent") == indicators.get("atr_percent") else "—")
    d.metric("Technical signal", signal.label)
    with st.expander("Why this analysis?"):
        for reason in signal.reasons:
            st.write(f"• {reason}")
        st.markdown("**Score risks**")
        for risk in ai.get("risks", []):
            st.write(f"• {risk}")
...
with company_tab:
    ...
    with left:
        ...
        for label, value in rows.items():
            if isinstance(value, (int, float)):
                if label in {"Profit margin", "Operating margin", "ROE", "Revenue growth", "Earnings growth"}:
                    value = f"{value:.1%}"
                elif label in {"Market cap", "Free cash flow"}:
                    value = f"{value:,.0f}"
                else:
                    value = f"{value:.2f}"
    ...
    with right:
        ...
        for label, key in [("P/E", "pe"), ("Forward P/E", "forward_pe"), ("PEG", "peg"), ("Price / Book", "price_to_book"), ("Dividend yield", "dividend_yield")]:
            ...
            text = f"{value:.2%}" if key == "dividend_yield" and isinstance(value, (int, float)) else f"{value:.2f}" if isinstance(value, (int, float)) else "—"
...
with analyst_tab:
    ...
    cols = st.columns(5)
    for col, label, key in zip(cols, ["Strong Buy", "Buy", "Hold", "Sell", "Strong Sell"], ["strong_buy", "buy", "hold", "sell", "strong_sell"]):
    ...
    for col, label, key in zip(cols, ["Low", "Mean", "Median", "High", "Current"], ["low", "mean", "median", "high", "current"]):
        ...
        col.metric(label, f"{value:.2f}" if isinstance(value, (int, float)) else "—")
...
with risk_tab:
    ...
    side = c1.selectbox("Position", ["LONG", "SHORT"])
    capital = c1.number_input("Capital", min_value=1.0, value=10000.0, step=500.0)
    entry = c2.number_input("Entry price", min_value=0.01, value=float(quote["latest_close"]), step=max(.01, float(quote["latest_close"]) / 100))
    leverage = c2.slider("Leverage", 1.0, 20.0, 3.0, .5)
    stop = c3.number_input("Stop loss (%)", min_value=.1, value=3.0, step=.5)
    target = c3.number_input("Take profit (%)", min_value=.1, value=6.0, step=.5)
    plan = leverage_plan(side, entry, leverage, stop, target, capital)
    p1, p2, p3, p4, p5 = st.columns(5)
    p1.metric("Notional", f"{plan['notional']:,.2f}")
    p2.metric("Units", f"{plan['units']:,.3f}")
    p3.metric("Stop", f"{plan['stop']:.2f}")
    p4.metric("Target", f"{plan['target']:.2f}")
    p5.metric("Reward / risk", f"{plan['reward_risk']:.2f}x")
    x, y = st.columns(2)
    x.metric("Approx. loss at SL", f"-{plan['max_loss']:,.2f}")
    y.metric("Approx. profit at TP", f"+{plan['potential_profit']:,.2f}")
